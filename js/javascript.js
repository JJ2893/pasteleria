...
//BASE DE DATOS
// pueden optar por usar un objeto, quizá al inicio se hará complicado, pero ya después es un poco más sencillo, pongo un ejemplo con un elemento
// var productos = {
//   "Pastel chocolate": {
//     imgG: "",
//     imgP: "",
...
// "Pas...."
// }
// o también usando índices como claves
// var productos = {
//   0: {
...
window.onload = function () {
  ...
  for (i in productos) {
    DIVS[i].innerHTML = '<a id="imgG' + i + '" href="' + imgGrandes[i] + '"><img id="imgP' + i + '" class="imagen chica" src="' + imgPeque[i] + '"></a><div class="etiquetas"><b><span id="pro' + i + '">' + productos[i] + '</span>: <span id="pre' + i + '">$' + precios[i] + '</span></b></div><div class="stock">Hay en existencia <span id="uni' + i + '">' + stock[i] + '</span> unidades,<br/>¿Cuantas quiere?: <input class="uniBien" type="number" id="uniUser' + i + '" name="uniUser" value="0" size="4" /></div>';

    // para que no hagan tanta concatenación, en JavaScript existen los template strings, que van con `` y dentro pueden interpolar variables usando ${}
    DIVS[i].innerHTML = `
      <a id="imgG${i}" href="${imgGrandes[i]}">
        <img id="imgP${i}" class="imagen chica" src="${imgPeque[i]}">
      </a>
      <div class="etiquetas">
        <b><span id="pro${i}">${productos[i]}<span>: <span id="pre${i}">$${precios[i]}</span></b>
      </div>
      <div class="stock">Hay en existencia <span id="uni${i}">${stock[i]}</span> unidades,<br> ¿Cuántas quiere?: <input "uniBien" type="number" id="uniUser${i}" name="uniUser" value="0" size="4" min="0"></div>`
  }
...
// Si en las funciones no van a utilizar los parametros, no los declaren en los paréntesis de la funcion
...
function validaLasUnidades(elEvento) {
  ...
  for (i in productos) {
    ...
    if (uniUser[i].value == "" || uniUser[i].value > stock[i] || uniUser[i].value < 0) {
      ...
      // para acceder a class, la manera adecuada es a través de classList
      // por ejemplo
      // document.getElementById("todo").classList.add("todoNo")
      // y para eliminar la clase es
      // document.getElementById("todo").classList.remove("todoNo")
      // también existe el toggle
      // document.getElementById("todo").classList.toggle("todoNo")
...
/**/			document.getElementById("divPago").className = "divsNo";
  ...
  // para limitar los decimales usen .toFixed(Numero de decimales en entero) = .toFixed(2)
